...
class StoreSpecs: QuickSpec {
    ...
    override func spec() {
        ...
        describe("#subscribe") {
            ...
            it("handles 'recursive' dispatch correctly") {
                let store = Store(reducer: StringReducer(), state: "", middleware: [statePrintMiddleware])
                let subscriber = StringSubscriber(store: store)
                store.subscribe(subscriber)
            }
...
class StringSubscriber: StoreSubscriber {
    ...
    func newState(state: String) {
        if count < 3 {
            count += 1
            store.dispatch(NewStringValue(value: state + "@"))
        }
    }
}
...
let statePrintMiddleware: Middleware = { dispatch, getState in
    return { next in
        return { action in
            print(getState())
